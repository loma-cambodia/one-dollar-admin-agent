<template>
  <q-card :style="{
    ...
    <q-bar dark class="bg-primary text-white">
      <span class="text-body2">{{ $t(Utils.getKey("Extend Deal Time")) }}</span>
      <q-space />
    ...
    <q-card-section class="q-pt-lg">
      <q-input :autofocus="true" v-model="deal.time_period" :label="$t(Utils.getKey('time_period'))" dense outlined maxlength="20"
        lazy-rules :rules="[
          val => !!val || $t(Utils.getKey('Field is required')),
          val => Utils.containsOnlyNumbers(val) || $t(Utils.getKey('only Number')),
        ]" />
    </q-card-section>
...
const $q = useQuasar();
const { saving, extendTime } = useDeals();
const deal = ref({ ...props.data });

async function onSubmit() {
  try {
    deal.value.deal_id = deal.value.id
    if (!Utils.containsOnlyCharacter(deal.value.name)) {
      $q.notify({
        position: "top-right",
        type: "negative",
    ...
    console.log('deals ========== ', deal.value)
    await extendTime({ ...deal.value });
    emit("onUpdated");
    $q.notify({
      position: "top-right",
      type: "positive",
