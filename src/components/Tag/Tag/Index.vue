<template>
      ...
      <q-card-section>
        ...
        >
          <template v-slot:top>
            ...
            <add-button
              v-if="Utils.hasPermissions(['Tag: Create Tag'])"
              :disable="loading"
              @click="showAdd = true"
              tooltip-text="Add"
              color="primary"
          ...
          <template v-slot:body-cell-actions="props">
            <q-td class="text-center">
              <q-btn
                v-if="Utils.hasPermissions(['Tag: Edit/Update Tag'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
              ...
              <q-btn
                v-if="Utils.hasPermissions(['Tag: Delete Tag'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
    ...
    <q-dialog v-model="showAdd" position="top" persistent>
      <add-tag :languages="languages" @onClose="showAdd = false" @onAdded="onRefresh" />
    </q-dialog>
...
import useTag from "../../composables/useTags";
import Utils from "../../helpers/Utils";

import Breadcrumbs from "../../components/Menu/BreadCrumbs.vue";
import AddButton from "../../components/Buttons/AddButton.vue";
import AddTag from "../../components/Tag/Add.vue";
import EditTag from "../../components/Tag/Edit.vue";
import Confirm from "../../components/Shared/Confirm.vue";
import Loading from "src/components/Shared/Loading.vue";
import useLanguage from "src/composables/useLanguage";
const { all } = useLanguage();

const { loading, columns, items, trash, paginate } = useTag();
const {
  showAdd,
  showEdit,
  selected,
