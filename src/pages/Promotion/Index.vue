<template>
      ...
      <q-card-section>
        ...
        >
          <template v-slot:top>
            ...
            <add-button
              v-if="Utils.hasPermissions(['Promotion: Create Promotion'])"
              :disable="loading"
              @click="showAdd = true"
              tooltip-text="Add"
              color="primary"
          ...
          <template v-slot:body-cell-actions="props">
            <q-td class="text-center">
              <q-btn
                v-if="Utils.hasPermissions(['Promotion: Edit/Update Promotion'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
              ...
              <q-btn
                v-if="Utils.hasPermissions(['Promotion: Delete Promotion'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
    ...
    <q-dialog v-model="showAdd" position="top" persistent>
      <add-promotion :languages="languages" @onClose="showAdd = false" @onAdded="onRefresh" />
    </q-dialog>
...
import AddPromotion from "../../components/Promotion/Add.vue";
...
const { loading, columns, items, trash, paginate } = usePromotion();
const {
  showAdd,
  showEdit,
  selected,
