<template>
  <q-page :class="!store.leftDrawerOpen ? 'ml-5' : ''">
      ...
      <q-card-section>
        ...
        >
          <template v-slot:top>
            ...
            <add-button
              v-if="Utils.hasPermissions(['WinnerGallery: Create WinnerGallery'])"
              :disable="loading"
              @click="showAdd = true"
              tooltip-text="Add"
              color="primary"
        ...
        <template v-slot:body-cell-description="props">
          ...
          <template v-slot:body-cell-actions="props">
            <q-td class="text-center">
              <q-btn
                v-if="Utils.hasPermissions(['WinnerGallery: Edit/Update WinnerGallery'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
    ...
    <q-dialog v-model="showAdd" position="top" persistent>
      ...
      <add-winner-gellery :languages="languages"  @onClose="showAdd = false" @onAdded="onRefresh" />
    </q-dialog>
...
import EditWinnerGellery from "../../components/WinnerGallery/Edit.vue";
import AddWinnerGellery from "../../components/WinnerGallery/Add.vue";
import Confirm from "../../components/Shared/Confirm.vue";
import useLanguage from "src/composables/useLanguage";

const { all } = useLanguage();
const locale = inject("locale");
const { loading, columns, items, trash, paginate } = useWinnerGallery();
const {
  showAdd,
  showEdit,
  selected,
